    ...
    return(
        <section id="waitlist_page" className="flex flex-col">
            ...
            <form action="#" onSubmit={handleFormSubmit}
                  className="xxs:w-3/4 2xl:w-5xl flex flex-col mx-auto xxs:my-10 md:my-15 lg:my-20 xl:my-25 2xl:my-30 space-y-5 ">
                {/* Error Message (if any)*/}
                {!isPhoneValid && <p className="text-center text-red-600">* Please enter a valid phone number</p>}
                {!isEmailValidFormat &&
                    <p className="text-left text-red-600 max-w-6xl mx-auto mb-5">* Please enter a valid email
                        format</p>}
                {/* organization name */}
                <input type="text" id="orgnizationName" placeholder="* Organization" className="form-input"
                       value={orgnizationName} onChange={(e) => setOrganizationName(e.target.value)} required/>
                <div className="flex xxs:space-y-5 md:space-y-0 md:space-x-5 xxs:flex-col md:flex-row">
                    ...
                    {/* last name*/}
                    <input type="form-input" id="lastName" placeholder="* Last Name" className="form-input"
                           value={lastName} onChange={(e) => setLastName(e.target.value)} required/>
                </div>
                <div className="flex xxs:space-y-5 md:space-y-0 md:space-x-5 xxs:flex-col md:flex-row">
                    {/* email */}
                    <input type="email" id="email" placeholder="* Email" className=" form-input" value={email}
                           onChange={(e) => setEmail(e.target.value)} required/>
                    {/* phone */}
                    <input type="text" id="phone" placeholder="* Phone Number" className="form-input" value={phone}
                           onChange={(e) => setPhone(e.target.value)} required/>
                </div>
                {/* subject line */}
                <input type="text" id="subject" placeholder="* Subject" className="form-input" value={subject}
                       onChange={(e) => setSubject(e.target.value)} required/>
                {/* message */}
                <textarea rows={5} id="message" className="form-input" placeholder="* Message" value={message}
                          onChange={(e) => setMessage(e.target.value)} required/>
                {/* Submit button */}
                <button type="submit"
                        className="font-body bg-secondary-base-3 xxs:w-1/5 sm:w-1/4 lg:w-1/3 xl:w-1/2 rounded-2xl py-2 self-center border-2 border-dark-border drop-shadow-secondary font-bold cursor-pointer mt-5 xxs:text-sm">SUBMIT
                </button>
            </form>
        </section>
    )
